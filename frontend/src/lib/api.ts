...
const API_BASE_URL = "https://demo-deployment-latest-mt9h.onrender.com";
  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (response.status === 400) {
        throw new ApiError(
          "Invalid message data. Please check your message text and try again.",
          400
        );
      }
      throw new ApiError(
        `Failed to create message: ${response.statusText}`,
        response.status
      );
    }
...
export const updateMessage = async (
  ...
  if (!response.ok) {
    ...
    throw new ApiError(
      `Failed to update message: ${response.statusText}`,
      response.status
    );
  }
...
export const getMessagesByUser = async (
  ...
  const response = await fetch(
    `${API_BASE_URL}/accounts/${accountId}/messages`
  );

  if (!response.ok) {
    throw new ApiError(
      `Failed to fetch user messages: ${response.statusText}`,
      response.status
    );
  }
